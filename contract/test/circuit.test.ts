...
describe("Setup", () => {
  ...
  describe("Ticket Swap tests", () => {
    ...
    before(async () => {
      ...
      backend = new BarretenbergBackend(circuit as unknown as any, {
        threads: 8,
      });
    ...
    const getClaimInputs = async ({ user }: { user: any }) => {
      ...
      const frArray: any[] = Array.from(signatureBuffer).map(
        (byte) => new Fr(BigInt(byte as any))
      );
    ...
    describe("Anonymous user", () => {
      ...
      it.skip("Verifies a valid claim on-chain", async () => {
        const { claimer_pub, hashed_message } = await getClaimInputs({
          user: user1,
        });
