  ...
  return (
    ...
    >
      ...
      {availableAddresses.map((acc) => (
        <Button onClick={() => signData(acc)}>
          Sign with connected account
        </Button>
      ))}

      {storedSignature && (
        <Text fontSize={"xs"}>
          Signature captured: for {shortenEthAddress(storedSignature!.account)}
        </Text>
      )}
      ...
      {storedSignature &&
        availableAddresses.map((acc) => (
          <>
            <Button onClick={() => claim(acc)} isLoading={loading}>
              Claim Ticket
            </Button>
            <br />
          </>
